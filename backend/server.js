...
app.post('/api/pedido', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      ...
      [new Date().toISOString(), cliente.nombre, cliente.email, cliente.telefono || '',
       cliente.provincia || '', cliente.direccion || '', JSON.stringify(items), total]
    );
    ...
    resend.emails.send({
      from: `GG'SNK Pedidos <pedidos@ggsnk.store>`, to: process.env.EMAIL_DESTINO,
    ...
    resend.emails.send({
      from: `GG'SNK <pedidos@ggsnk.store>`, to: cliente.email,
...
app.put('/api/pedido/:id', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      `UPDATE pedidos SET estado = COALESCE($1, estado), tracking = COALESCE($2, tracking),
       notas = COALESCE($3, notas), costo = COALESCE($4, costo) WHERE id = $5`,
...
app.post('/api/webhook/mercadopago', async (req, res) => {
  ...
  try {
    ...
    const cliente = { nombre: p.nombre, email: p.email, telefono: p.telefono, provincia: p.provincia, direccion: p.direccion };
    resend.emails.send({
      from: `GG'SNK Pedidos <pedidos@ggsnk.store>`, to: process.env.EMAIL_DESTINO,
...
app.get('/api/seguimiento/:id', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      'SELECT id, fecha, productos, total_ars, estado, tracking FROM pedidos WHERE id = $1', [id]
    );
    ...
    res.json({
      ok: true,
      pedido: {
        id: p.id, fecha: p.fecha, estado: p.estado, tracking: p.tracking,
        total_ars: p.total_ars, cantidad: productos.length,
